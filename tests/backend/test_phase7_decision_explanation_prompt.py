...
def test_decision_explanation_prompt_builder_includes_grounding_rules() -> None:
    decision = _build_decision()
    context = _build_context()
    builder = DecisionExplanationPromptBuilder()

    request = builder.build_request(decision, context)

    assert request.operation == "decision_explanation"
    assert request.output_format.value == "json"
    assert request.metadata["job_id"] == "job_001"
    assert request.metadata["target_drug"] == "tetracycline"
    ...
    assert "Never change the triage outcome" in system_message
    assert "Do not invent mechanisms" in system_message
    assert "Do not use clinical directive language" in system_message
    assert "Keep the payload compact enough to finish in one response" in system_message
    assert "next_steps must always be a JSON array of strings" in system_message
    assert "\"block_id\":\"decision_summary\"" in system_message
    assert "decision_summary" in user_message
    assert "prohibited_inference_zones" in user_message
    assert "confirm phenotype in downstream review flow" in user_message


def test_decision_explanation_prompt_builder_collects_allowed_evidence_ids() -> None:
    builder = DecisionExplanationPromptBuilder()
...
def test_decision_explanation_prompt_builder_rejects_context_drift() -> None:
    decision = _build_decision()
    context = _build_context().model_copy(update={"job_id": "job_999"})

    with pytest.raises(ValueError, match="job_id"):
        DecisionExplanationPromptBuilder().build_request(decision, context)


@pytest.mark.live
def test_live_decision_explanation_prompt_returns_grounded_copilot_response() -> None:
    settings = load_settings()
    assert settings.llm.provider == "openrouter", (
        "LLM_PROVIDER must be set to openrouter for live explanation prompt tests"
    )
    ...
    try:
        ...
        if "HTTP 429" in str(exc):
            pytest.skip("OpenRouter rate-limited the live explanation smoke test")
        raise
